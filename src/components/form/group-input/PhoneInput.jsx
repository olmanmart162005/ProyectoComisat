...
const PhoneInput = ({
  ...
  // Sync with value prop if provided (useful for edit mode)
  useEffect(() => {
    if (value) {
  ...
  const handleCountryChange = (e) => {
    ...
    // Usually changing country doesn't wipe the number but might change the format.
    // For now we just keep the number as is, but we could notify the change.
  };
  ...
  return (
    <div className="relative flex">
      ...
      <input
        ...
        } rounded-lg border border-gray-200 bg-white px-3.5 py-2.5 text-sm text-gray-900 outline-none transition-all focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 dark:border-white/10 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800`}
      />
