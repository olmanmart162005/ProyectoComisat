...
export default function DashboardRRHH() {
  ...
  const colores = ["#8884d8", "#82ca9d", "#ffc658", "#ff7c7c", "#8dd1e1", "#d084d0", "#f7b500"];
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        // Bajas de este mes
        const historialSnap = await getDocs(collection(db, "historialEmpleados"));
        ...
        empleadosSnap2.docs.forEach((doc) => {
          const fechaRegistro = doc.data().fechaRegistro?.toDate?.();
          if (fechaRegistro && fechaRegistro.getFullYear() === today.getFullYear()) {
        ...
        const chartData = Object.keys(departamentosCount).map((departamento) => ({
          name: departamento,
        ...
        <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow border border-gray-200 dark:border-gray-700">
          ...
          <div className="text-gray-600 dark:text-gray-300">
            ...
            >
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="nuevos" name="Empleados nuevos" stroke="#3b82f6" />
              <Line type="monotone" dataKey="bajas" name="Bajas" stroke="#ef4444" />
            </LineChart>
          </div>
        </div>
      )}
