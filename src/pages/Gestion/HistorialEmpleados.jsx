...
import { COLUMNAS_EXPORT_HISTORIAL, historialEmpleadoColumns } from "./columns/historialEmpleadoColumns";
...
export default function HistorialEmpleados() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-800 dark:text-white/90">Historial de Empleados</h2>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 md:gap-6">
        <MetricCard title="Total en Historial" value={historial.length} icon={<GroupIcon className="text-gray-800 size-6 dark:text-white/90" />} iconWrapperClass="bg-gray-100 dark:bg-gray-800" />
        ...
        <MetricCard title="Usuarios Eliminados" value={totalUsuariosEliminados} icon={<CheckCircleIcon className="text-blue-600 size-6 dark:text-blue-400" />} iconWrapperClass="bg-blue-50 dark:bg-blue-500/10" />
      </div>
      ...
      <DataTable columns={columns} data={historialFiltrado} loading={loading}>
        <DataTable.Toolbar searchPlaceholder="Buscar empleado...">
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto sm:ml-auto">
            <select value={filtroDepartamento} onChange={(e) => setFiltroDepartamento(e.target.value)} className={`w-full sm:w-52 ${selectClass}`}>
              <option value="" className="bg-white text-gray-900">Departamento</option>
              {departamentos.map((dep) => (
                <option key={dep} value={dep} className="bg-white text-gray-900">{dep}</option>
              ))}
            </select>
            ...
            <ExportButtons
              ...
              filename={"Historial Empleados " + new Date().toLocaleDateString("es-HN")}
              sheetName="Historial de Empleados"
              meta={{
                empresa: "Comisariato San Jose",
                usuario: nombreEmpleado || "Sistema",
                extra: filtroDepartamento ? `Departamento: ${filtroDepartamento}` : "Listado completo",
              }}
              pdfOptions={{ title: "Historial de Empleados", subtitle: new Date().toLocaleDateString("es-HN") }}
              onExport={(formato) =>
                registrarBitacora({
                  usuario: user.email,
